...
public:
  ...
  // EnQuantize:
  inline void EnQuantize(const F32* pPosition, unsigned short* pQuantPosition);
  // EnQuantize:
  inline void EnQuantize(const F32* pPosition, I32* pQuantPosition, int fc);
  // DeQuantize:
  inline void DeQuantize(const I32* pQuantPosition, F32* pPosition);
  // DeQuantize:
  inline void DeQuantize(const unsigned short* pQuantPosition, F32* pPosition);
  // DeQuantize:
  inline void DeQuantize(const unsigned short* pQuantPosition, F32* pPosition, int pos);
...
{
  pQuantPosition[0] = I32(m_dEnQuantizeMultiplier * ((F64)pPosition[0] - (F64)m_afMin[0]) + 0.5); // is th at casting a good idea? jack says ... it is important to avoid bit cancellation
...
// EnQuantize: 16bits
//-----------------------------------------------------------------------------
inline void PositionQuantizerNew::EnQuantize(const F32* pPosition, unsigned short* pQuantPosition)
{
  pQuantPosition[0] = (unsigned short)(m_dEnQuantizeMultiplier * ((F64)pPosition[0] - (F64)m_afMin[0]) + 0.5); // is th at casting a good idea? jack says ... it is important to avoid bit cancellation
  pQuantPosition[1] = (unsigned short)(m_dEnQuantizeMultiplier * ((F64)pPosition[1] - (F64)m_afMin[1]) + 0.5);
  pQuantPosition[2] = (unsigned short)(m_dEnQuantizeMultiplier * ((F64)pPosition[2] - (F64)m_afMin[2]) + 0.5);
...
// EnQuantize:
//-----------------------------------------------------------------------------
inline void PositionQuantizerNew::EnQuantize(const F32* pPosition, I32* pQuantPosition, int fc)
{
	if(fc == 0)
	{
  pQuantPosition[0] = I32(floor(m_dEnQuantizeMultiplier * ((F64)pPosition[0] - (F64)m_afMin[0]))); // is th at casting a good idea? jack says ... it is important to avoid bit cancellation
  pQuantPosition[1] = I32(floor(m_dEnQuantizeMultiplier * ((F64)pPosition[1] - (F64)m_afMin[1])));
  pQuantPosition[2] = I32(floor(m_dEnQuantizeMultiplier * ((F64)pPosition[2] - (F64)m_afMin[2])));
	}
	else
	{
  pQuantPosition[0] = I32(ceil(m_dEnQuantizeMultiplier * ((F64)pPosition[0] - (F64)m_afMin[0]))); // is th at casting a good idea? jack says ... it is important to avoid bit cancellation
  pQuantPosition[1] = I32(ceil(m_dEnQuantizeMultiplier * ((F64)pPosition[1] - (F64)m_afMin[1])));
  pQuantPosition[2] = I32(ceil(m_dEnQuantizeMultiplier * ((F64)pPosition[2] - (F64)m_afMin[2])));
	}
...
// DeQuantize: 16bit
//-----------------------------------------------------------------------------
inline void PositionQuantizerNew::DeQuantize(const unsigned short* pQuantPosition, F32* pPosition)
{
  pPosition[0] = (F32)(m_dDeQuantizeMultiplier*pQuantPosition[0] + (F64)m_afMin[0]);// is that casting a good idea? jack says ... yes.
  pPosition[1] = (F32)(m_dDeQuantizeMultiplier*pQuantPosition[1] + (F64)m_afMin[1]);
  pPosition[2] = (F32)(m_dDeQuantizeMultiplier*pQuantPosition[2] + (F64)m_afMin[2]);
...
//-----------------------------------------------------------------------------
// DeQuantize: 16bit, 1 position
//-----------------------------------------------------------------------------
inline void PositionQuantizerNew::DeQuantize(const unsigned short* pQuantPosition, F32* pPosition, int pos)
{
  pPosition[pos] = (F32)(m_dDeQuantizeMultiplier*pQuantPosition[pos] + (F64)m_afMin[pos]);// is that casting a good idea? jack says ... yes.
}
//-----------------------------------------------------------------------------
