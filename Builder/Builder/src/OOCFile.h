...

{
	...
	if (! (m_hFile = CreateFile(pFileName, m_FileMappingMode, m_FileShareMode, NULL, m_FileOpenMode,
		...
		if (pMappingMode && strchr (pMappingMode, 'c')) {
			// compute filesize
			m_fileSize = MaxNumElements * sizeof (T);
...
// main access operator, i is byte offset!
template <class T>
FORCEINLINE T& OOCFile<T>::GetRef (unsigned int i)
{
	// find pos in table for this i
	unsigned int startOffset = (i>>m_BlockSizePowerTwo);
	unsigned int tablePos = startOffset & m_CacheNumMask;
...
void FORCEINLINE OOCFile<T>::load(unsigned int tablePos) {
	...
	if (!(m_CacheTable[tablePos].address = (char *)MapViewOfFile(m_hMapping, m_FileMappingMode, 0, startAddr, blockSize))) {
...
#else
	m_CacheTable[tablePos].address = (char *)MapViewOfFile(m_hMapping, m_FileMappingMode, 0, startAddr, blockSize);	
#endif
}
