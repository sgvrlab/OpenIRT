...
/**
 * BVH class for high-level tree of voxels
 * 
 */
class VoxelBVH {
public:
	...
	* the tree for.
	*/
	VoxelBVH(Voxel *tris, unsigned int numTris, Vector3 &min, Vector3 &max, Vector3 *voxelMins, Vector3 *voxelMaxs) {
		...
		this->voxelMins = voxelMins;
		this->voxelMaxs = voxelMaxs;
	}
	...
	BSPArrayTreeNode writeVoxelBVH(Voxel &voxel, HANDLE nodePtr, HANDLE idxPtr, LARGE_INTEGER &nodeOffset, LARGE_INTEGER &idxOffset, const char* filename);
...
protected:
	...
	FORCEINLINE void updateMinBB(Vector3 &min, Vector3 &vec)
	{
		min.e[0] = ( min.e[0] < vec.e[0] ) ? min.e[0] : vec.e[0];
		min.e[1] = ( min.e[1] < vec.e[1] ) ? min.e[1] : vec.e[1];
		min.e[2] = ( min.e[2] < vec.e[2] ) ? min.e[2] : vec.e[2];
	...
	FORCEINLINE void updateMaxBB(Vector3 &max, Vector3 &vec)
	{
		max.e[0] = ( max.e[0] > vec.e[0] ) ? max.e[0] : vec.e[0];
		max.e[1] = ( max.e[1] > vec.e[1] ) ? max.e[1] : vec.e[1];
		max.e[2] = ( max.e[2] > vec.e[2] ) ? max.e[2] : vec.e[2];
	...
	bool Subdivide(long myOffset, VoxelIndexList *trilist, int depth, FILE *indexFP, FILE *nodeFP);
	/**
