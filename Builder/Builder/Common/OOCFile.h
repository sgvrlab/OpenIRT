...
// sungeui -------------------------------
// specialized code for collecting data.



// main access operator, i is byte offset!
FORCEINLINE const LODNode& OOCFile<LODNode>::operator[](unsigned int i) {
...
#ifdef WORKING_SET_COMPUTATION
	g_LODSim.Access (i / sizeof (LODNode));
#endif

#ifdef OOCFILE_DEBUG
	assert(i < m_fileSize);
	assert(i % sizeof(LODNode) == 0);
	...
	if (m_CacheTable[tablePos].address != NULL && m_CacheTable[tablePos].fileStartOffset == startOffset) {
		// yes: return pointer
		return *((LODNode *)(m_CacheTable[tablePos].address + (i & m_BlockMaskToOffset)));
	}
	else {
#ifdef OOCFILE_PROFILE
		cacheMisses++;
...
FORCEINLINE const _Vector4& OOCFile<_Vector4>::operator[](unsigned int i) {
...
#ifdef WORKING_SET_COMPUTATION
	g_VerSim.Access (i / sizeof (_Vector4));
#endif

#ifdef OOCFILE_DEBUG
	assert(i < m_fileSize);
	assert(i % sizeof(_Vector4) == 0);
	...
	if (m_CacheTable[tablePos].address != NULL && m_CacheTable[tablePos].fileStartOffset == startOffset) {
		// yes: return pointer
		return *((_Vector4 *)(m_CacheTable[tablePos].address + (i & m_BlockMaskToOffset)));
	}
	else {
#ifdef OOCFILE_PROFILE
		cacheMisses++;
...
#ifdef WORKING_SET_COMPUTATION
	g_kdIdxSim.Access (i / sizeof (unsigned int));
#endif
