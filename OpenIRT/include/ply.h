...
#else
extern __declspec(dllimport) FILE* fopenGzipped(const char* filename, const char* mode);
#endif
...
#define PLY_LITTLE_ENDIAN_PLATFORM
#undef PLY_BIG_ENDIAN_PLATFORM
#else
#define PLY_BIG_ENDIAN_PLATFORM
#undef PLY_LITTLE_ENDIAN_PLATFORM
#endif
...
typedef struct PlyElement_t {     /* description of an element */
  ...
  PlyElement_t() : name(0), props(0), store_prop(0) {}
} PlyElement;
...
typedef struct PlyFile_t {        /* description of PLY file */
  ...
  PlyFile_t() : fp(0), elems(0), comments(0), obj_info(0), which_elem(0), other_elems(0), current_rules(0), rule_list(0) {}
} PlyFile;
...
void printtime();
void gohome();
PlyFile* open_ply_for_write(char*, int, char**, int);
PlyFile* open_ply_for_read(char*);
