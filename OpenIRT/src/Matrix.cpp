...
// transform a vector by matrix
Vector4 operator* (const Matrix & left_op, const Vector4 & right_op)
{
	Vector4 ret;

	ret[0] = right_op[0] * left_op.x[0][0] + right_op[1] * left_op.x[0][1] + right_op[2] * left_op.x[0][2] + right_op[3] * left_op.x[0][3];
	ret[1] = right_op[0] * left_op.x[1][0] + right_op[1] * left_op.x[1][1] + right_op[2] * left_op.x[1][2] + right_op[3] * left_op.x[1][3];
	ret[2] = right_op[0] * left_op.x[2][0] + right_op[1] * left_op.x[2][1] + right_op[2] * left_op.x[2][2] + right_op[3] * left_op.x[2][3];
	ret[3] = right_op[0] * left_op.x[3][0] + right_op[1] * left_op.x[3][1] + right_op[2] * left_op.x[3][2] + right_op[3] * left_op.x[3][3];

	if(ret[3] != 0.0f) ret /= ret[3];
	return ret;
}
...
bool operator== (const Matrix& left_op, const Matrix& right_op)
{
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < 4; j++)
			if(left_op.x[i][j] != right_op.x[i][j])
				return false;
	return true;
}
...
RayTransform identityTransform(0,0,0);
};