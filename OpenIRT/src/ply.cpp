...
void write_binary_item_BE(FILE *, int, unsigned int, double, int);
void write_binary_item_LE(FILE *, int, unsigned int, double, int);
...
void get_binary_item_BE(FILE *, int, int *, unsigned int *, double *);
void get_binary_item_LE(FILE *, int, int *, unsigned int *, double *);

/* get a bunch of elements from a file */
void ascii_get_element(PlyFile *, char *);
void binary_get_element_BE(PlyFile *, char *);
void binary_get_element_LE(PlyFile *, char *);
...
PlyFile* open_ply_for_read(char* file_name)
{
	PlyFile* ply;
	FILE* fp;

	/*	if (file_name[strlen(file_name)-1] == 'z')
	{
	fp = fopenGzipped(file_name,"rb");
	if (fp == 0)
	{
	...
	if (ply == 0)
	{
	printf("corrupt ply file for read.\n");
	gohome();
	}
	}
	else
...
PlyFile* open_ply_for_write(char* file_name, int elem_num, char** elem_names, int file_type)
{
	PlyFile* ply;
	FILE* fp;
	...
	ply = write_ply(fp, elem_num, elem_names, file_type);
	if (ply == 0)
	{
		printf("corrupt ply file for read.\n");
		gohome();
	...
	)
{
	...
	int size = (int)(sizeof (char) * (strlen (filename) + 5));
	name = (char *) myalloc (size);
	strcpy_s(name, size, filename);
	if (strlen (name) < 4 ||
		strcmp (name + strlen (name) - 4, ".ply") != 0)
		strcat_s(name, size, ".ply");
...
void put_element_ply(PlyFile *plyfile, void *elem_ptr)
{
	...
	if (plyfile->file_type == PLY_ASCII) {
	...
	else if (plyfile->file_type == PLY_BINARY_BE) {

		/* write a binary Big Endian file */
		...
		for (j = 0; j < elem->nprops; j++) {
			...
			if (prop->is_list == PLY_LIST) {   /* list */
				...
				get_stored_item ((void *) item, prop->count_internal,
					&int_val, &uint_val, &double_val);
				write_binary_item_BE(fp, int_val, uint_val, double_val,
					prop->count_external);
				list_count = uint_val;
				item_ptr = (char **) (elem_data + prop->offset);
				item = item_ptr[0];
		...
		/* write a binary Little Endian file */
		...
		for (j = 0; j < elem->nprops; j++) {
			...
			if (prop->is_list == PLY_LIST) {   /* list */
				...
				get_stored_item ((void *) item, prop->count_internal,
					&int_val, &uint_val, &double_val);
				write_binary_item_LE(fp, int_val, uint_val, double_val,
					prop->count_external);
				list_count = uint_val;
				item_ptr = (char **) (elem_data + prop->offset);
				item = item_ptr[0];
...
void ply_get_element(PlyFile *plyfile, void *elem_ptr)
{
	...
	{
		binary_get_element_BE(plyfile, (char *) elem_ptr);
	}
	else
	{
		binary_get_element_LE(plyfile, (char *) elem_ptr);
	}
}
...
int equal_strings(char *s1, char *s2)
{
	...
	//return (1);// there is a case that we have some special character (13) at the end of the words
	if (*s1 != *s2)
		return (0);
	else
		return (1);
...
char *recreate_command_line (int argc, char *argv[])
{
	...
	for (i = 0; i < argc; i++) {
		strcat_s(line, sizeof(char) * len, argv[i]);
		if (i != argc - 1)
			strcat_s(line, sizeof(char) * len, " ");
	}
...
void binary_get_element_BE(PlyFile *plyfile, char *elem_ptr)
{
	int j,k;
	PlyElement *elem;
	PlyProperty *prop;
	...
	for (j = 0; j < elem->nprops; j++) {
		...
		if (prop->is_list == PLY_LIST) {          /* list */
			...
			/* get and store the number of items in the list */
			get_binary_item_BE(fp, prop->count_external,
				&int_val, &uint_val, &double_val);
			if (store_it) {
				item = elem_data + prop->count_offset;
				store_item(item, prop->count_internal, int_val, uint_val, double_val);
			...
			if (list_count == 0) {
				...
				/* read items and store them into the array */
				for (k = 0; k < list_count; k++) {
					get_binary_item_BE(fp, prop->external_type,
						&int_val, &uint_val, &double_val);
					if (store_it) {
						store_item (item, prop->internal_type,
							int_val, uint_val, double_val);
...
void binary_get_element_LE(PlyFile *plyfile, char *elem_ptr)
{
	int j,k;
	PlyElement *elem;
	PlyProperty *prop;
	...
	for (j = 0; j < elem->nprops; j++) {
		...
		if (prop->is_list == PLY_LIST) {          /* list */
			...
			/* get and store the number of items in the list */
			get_binary_item_LE(fp, prop->count_external,
				&int_val, &uint_val, &double_val);
			if (store_it) {
				item = elem_data + prop->count_offset;
				store_item(item, prop->count_internal, int_val, uint_val, double_val);
			...
			if (list_count == 0) {
				...
				/* read items and store them into the array */
				for (k = 0; k < list_count; k++) {
					get_binary_item_LE(fp, prop->external_type,
						&int_val, &uint_val, &double_val);
					if (store_it) {
						store_item (item, prop->internal_type,
							int_val, uint_val, double_val);
...
#if defined(PLY_LITTLE_ENDIAN_PLATFORM)
	char swap;
	char *c;
#endif
...
#if defined(PLY_BIG_ENDIAN_PLATFORM)
	char swap;
	char *c;
#endif
...
PlyFile *read_ply(FILE *fp)
{
	...
	{
		if(elem_names[i]) free(elem_names[i]);
	}
	if(num_elems > 0 && elem_names) free(elem_names);
...
void free_ply(PlyFile *plyfile)
{
	...
	if(plyfile->elems)
	{
		for(int i=0;i<plyfile->num_elem_types;i++)
		{
			...
			if(elem->nprops > 0 && elem->props) free(elem->props);
			if(elem->name) free(elem->name);
			if(elem->store_prop) free(elem->store_prop);
			free(elem);
		}
		free(plyfile->elems);
	}
	if(plyfile->comments) free(plyfile->comments);
	if(plyfile->obj_info) free(plyfile->obj_info);
